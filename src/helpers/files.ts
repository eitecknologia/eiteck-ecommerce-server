...
const imagesPath = "eiteck/cobos_ecommerce/images";

/* Upload a file in cloudinary Function */
export const uploadFiles = (
  file: fileUpload.UploadedFile | fileUpload.UploadedFile[] | undefined,
  validatedExt = [
    "png",
    "jpg",
    "jpeg",
  ...
  return new Promise(async (resolve, reject) => {
    try {
      ...
      if (!validatedExt.includes(extension)) {
        return reject(
          `La extensión ${extension} no es permitida, Extensiones Validas: ${validatedExt}`
        );
      }

      /* Upload the image in cloudinary */
      const { secure_url } = await cloudinary.uploader.upload(tempFilePath, {
        upload_preset: "cobos_ecommerce",
      });
