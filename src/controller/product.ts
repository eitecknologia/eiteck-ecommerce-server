...
export const getProductsByCategory = async (req: Request, res: Response) => {
    try {
        ...
        const subcategoriesIdsArray = subcategoriesInCategory.map((subcategory) => subcategory.subcategoryid);
        ...
        /* Calculate the total of pages */
        const total = await Product.count({ where: { isactive: true, productid: { [Op.in]: productsIdsArray } } })
        const totalPages = (Math.ceil(total / limit));
        const info = await infoPaginate(totalPages, total, pageSend, sizeSend);
        ...
        await sequelize.transaction(async (t) => {
            ...
            if (subcategories.length > 0) {
                for (const subcategoryid of subcategories) {
                    ...
                    if (!existRegister) {
                        await SubcategoryProducts.create({ productid: +productid, subcategoryid }, { transaction: t })
                    }
                }
            }
        });
