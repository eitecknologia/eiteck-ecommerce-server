...
import { DiscountCode, Product, ProductImages, Role, ShoppingCart } from "../models";
...
export const getMyShoppingCart = async (req: Request, res: Response) => {
    try {
        ...
        if (discountcode) {
            /* Get discount code */
            const discount = await DiscountCode.findOne({ where: { discountcode: (discountcode as string) } });
        ...
        if (discountDetails.ok && discountDetails.discountpercent) {
            totalCartDiscount = Number(((totalCart - (totalCart * discountDetails.discountpercent) / 100)).toFixed(2));
        }
        const ivaTotal = (discountDetails.ok) ? Number((totalCartDiscount * IVA).toFixed(2)) : Number((totalCart * IVA).toFixed(2));
        const totalSale = (discountDetails.ok) ? Number((totalCartDiscount + ivaTotal).toFixed(2)) : Number((totalCart + ivaTotal).toFixed(2));
...
/* Get my shopping cart Function */
export const updateQuantityShoppingCart = async (req: Request, res: Response) => {
    try {
...
export const deleteRegisterOfShoppingCart = async (req: Request, res: Response) => {
    try {
